...
- (nonnull instancetype)initWithKernelWidth:(NSUInteger)kernelWidth
  ...
  _convDesc.fusedNeuronDescriptor =
      [MPSNNNeuronDescriptor cnnNeuronDescriptorWithType:MPSCNNNeuronTypeNone];

  _sizeBiases = _outputFeatureChannels * sizeof(float);
  NSUInteger lenWeights = _inputFeatureChannels * _kernelHeight * _kernelWidth *
  ...
  _vDescWeights =
      [MPSVectorDescriptor vectorDescriptorWithLength:lenWeights
  ...
  _convWtsAndBias = [[MPSCNNConvolutionWeightsAndBiasesState alloc]
      initWithWeights:_weightVector.data
  ...
  MPSMatrixRandomDistributionDescriptor *randomDesc =
      [MPSMatrixRandomDistributionDescriptor
          uniformDistributionDescriptorWithMinimum:-0.2f
                                           maximum:0.2f];
  MPSMatrixRandomMTGP32 *randomKernel =
      [[MPSMatrixRandomMTGP32 alloc] initWithDevice:gDevice
